 ...
 * @author Heshitha
 */
@WebServlet(name = "GetPOI", urlPatterns = {"/GetPOI"})
public class GetPOI extends HttpServlet {
        ...
        try { 
          Gson gson = new Gson();
            ...
            while ((str = br.readLine()) != null) {
                sb.append(str);
                POIDetails poi= gson.fromJson(sb.toString(), POIDetails.class);
               ...
               //test 
               String getquery = "SELECT * from poi WHERE pid='"+poi.getPid()+"'";
                ...
                while (res.next()) {
                   ...
                   poid.setComments(getComments(con,res.getString("pid")));
                   poid.setVotes(res.getString("votes"));
                   poid.setAddress(res.getString("address"));
    ...
    public ArrayList<Comment> getComments(java.sql.Connection con,String pid){
        try {
            ResultSet res2 = DBHandle.getData(con,"Select * from comment c join test t where c.userid=t.id AND pid='"+pid+"'");
            ArrayList<Comment> cmntList;
            cmntList = new ArrayList<Comment>();
            while(res2.next()){
            Comment cmnt=new Comment();
            cmnt.setCommentID(res2.getString("cid"));
            cmnt.setPoiID(res2.getString("pid"));
            cmnt.setUserID(res2.getString("userName"));
            cmnt.setDate(res2.getString("date"));
            cmnt.setTime(res2.getString("time"));
            cmnt.setComment(res2.getString("comment"));
            cmntList.add(cmnt);
